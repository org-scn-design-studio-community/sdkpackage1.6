...
define(["css!../ZenCrosstabFix.css"], function() {
	var componentInfo = {
		...
		author : "Mike Howles",
		description : "A Fiori-Inspired Tile Container using UI5 Handler",
		topics : [{
			title : "SDK Component",
	 ...
	 * End of experiment
	 */
	sap.m.TileContainer.extend("org.scn.community.m.basics.TileContainer", {
		_itemConfig : [],
		_selectedTile : "",
		renderer : {},
		metadata : meta,
		...
		initDesignStudio : function() {
			...
			this.addStyleClass("TileContainer");
			this.drawTiles();
		},
		tileSelect : function(title,oControlEvent){
			this._selectedTile = title;
			this.fireDesignStudioPropertiesChangedAndEvent(["selectedTile"],"onclick");
		},
		/**
		...
		drawTiles : function(){
			...
			for(var i=0;i<this._itemConfig.length;i++){
				var tileConf = this._itemConfig[i];
				...
				tile.attachPress(function(t){return function(oControlEvent){
					this.tileSelect(t,oControlEvent);};
				}(tileConf.title),this);
				this.addTile(tile);
			}
		}
